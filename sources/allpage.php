<?php
    ...
    $banner = $cache->get("select photo from #_photo where type = ? and act = ? limit 0,1", array('logo1','photo_static'), 'fetch', 7200);
    $slogan = $cache->get("select name$lang from #_static where type = ? and find_in_set('hienthi',status) limit 0,1", array('slogan'), 'fetch', 7200);
    ...
    $slogan2 = $cache->get("select name$lang from #_static where type = ? and find_in_set('hienthi',status) limit 0,1", array('slogan2'), 'fetch', 7200);
    $slogan3 = $cache->get("select name$lang from #_static where type = ? and find_in_set('hienthi',status) limit 0,1", array('slogan3'), 'fetch', 7200);
    ...
    $logo1 = $cache->get("select id, photo, options from #_photo where type = ? and act = ? limit 0,1", array('logo1','photo_static'), 'fetch', 7200);
    ...
    $splist1 = $cache->get("select name$lang, slugvi, slugen, id from #_product_list where type = ? and find_in_set('hienthi',status) order by numb,id desc", array('mau-thiet-ke-dep'), 'result', 7200);
    $camnhan_khachhang = $cache->get("select name$lang, slugvi, slugen, desc$lang, date_created,content$lang, id, photo from #_news where type = ? and find_in_set('hienthi',status)  order by numb,id desc", array('camnhan-khachhang'), 'result', 7200);
    $social_footer = $cache->get("select name$lang, photo, link from #_photo where type = ? and find_in_set('hienthi',status) order by numb,id desc", array('social-footer'), 'result', 7200);
    $qrcode = $cache->get("select id, photo,link, options from #_photo where type = ? and act = ? limit 0,1", array('qrcode','photo_static'), 'fetch', 7200);
