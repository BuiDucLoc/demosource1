<?php
	...
	$requick = array(
		...
		array("tbl" => "", "field" => "id", "source" => "", "com" => "dat-theo-yeu-cau", "type" => "", "menu" => true),
	);
	...
	switch($com)
	{
		...
		case 'dat-theo-yeu-cau':
			$source = "contact";
			$template = "contact/contact1";
			$seo->set('type','object');
			$titleMain = 'Đặt theo yêu cầu';
			break;
		...
		case 'du-an':
			$source = "news";
			$template = isset($_GET['id']) ? "duan/duan_detail" : "duan/duan";
			$seo->set('type',isset($_GET['id']) ? "article" : "object");
			$type = $com;
			$titleMain = "Dự án";
			break;
